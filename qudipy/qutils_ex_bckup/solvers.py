...
# Directory to save debugging data
data_dir_save = 'python_data'
...
def build_2DSE_hamiltonian(consts, gparams):
    ...
    data_path_save = os.path.join(data_dir_save, 'B')
    sparse.save_npz(data_path_save, B)

    # data_path_save = os.path.join(data_dir_save, 'B_block')
    # sparse.save_npz(data_path_save, B)

    data_path_save = os.path.join(data_dir_save, 'PE_2D')
    sparse.save_npz(data_path_save, PE_2D)


    data_path_save = os.path.join(data_dir_save, 'KE_2D')
    sparse.save_npz(data_path_save, KE_2D)
...
def solve_many_elec_SE(gparams, n_elec, n_xy_ho, n_se=7, n_sols=4,
    ...
    new_hos, a_mat, lcho_ens = ex.basis_transform(gparams, origin_hos,
                                                           consts=consts,
                                                           unitary=True,
                                                           ortho_basis=True)


    data_path_save = os.path.join(data_dir_save, f'new_hos_{n_xy_ho[0]}_{n_xy_ho[1]}.txt')
    np.savetxt(data_path_save, np.real(new_hos))

    data_path_save = os.path.join(data_dir_save, f'a_mat_{n_xy_ho[0]}_{n_xy_ho[1]}.txt')
    np.savetxt(data_path_save, np.real(a_mat))

    data_path_save = os.path.join(data_dir_save, f'lcho_ens_{n_xy_ho[0]}_{n_xy_ho[1]}.txt')
    np.savetxt(data_path_save, np.real(lcho_ens))
    ...
    if ho_cmes:
        ...
        try:
            ho_cmes = np.load(os.path.join(cme_dir, f'CMEs_{n_xy_ho[0]}x{n_xy_ho[1]}.npy'),
                            mmap_mode='c')
            # @zmerino
            # cmes_ojb = scipy.io.loadmat('CMEsOrigin_11_11_1.00.mat')
            # ho_cmes = cmes_ojb['CMEs_lib']
            ...
            ho_cmes = ex.calc_origin_cme_matrix(n_xy_ho[0], n_xy_ho[1],
                                                consts=consts)
            np.save(os.path.join(cme_dir, f'CMEs_{n_xy_ho[0]}x{n_xy_ho[1]}.npy'), ho_cmes)
    ...
    data_path_save = os.path.join(data_dir_save, f'tran_cme_{n_xy_ho[0]}_{n_xy_ho[1]}.txt')
    np.savetxt(data_path_save, se_cmes)
    ...
    # @zmerino
    data_path_save = os.path.join(data_dir_save, f'2nd_quantized_H_{n_xy_ho[0]}_{n_xy_ho[1]}.txt')
    np.savetxt(data_path_save, ham_2q.view(float))
